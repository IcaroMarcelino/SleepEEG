...
output.write('K, Exec, Precision_S, Precision_NS, Recall_S, Recall_NS, Fscore_S, Fscore_NS, Support_S, Support_NS, Acc\n')

for k in [3,5,7,9,11,13,15,17,19]:
	result = []
	for i in range(100):
		X_train1, y_train1, X_test1, y_test1 = import_all_data(files,1,.3)
		knn = KNeighborsClassifier(n_neighbors=k)
		knn.fit(X_train1, y_train1)
		pred = knn.predict(X_test1)
		acc = precision_recall_fscore_support(y_test1, pred)
		acc1 = accuracy_score(y_test1, pred)
		result.append(acc)
		output.write(str(k) + ',' + str(i) + ',' + str(acc[0][0]) + ',' + str(acc[0][1]) + ',' + str(acc[1][0])  + ',' + str(acc[1][1]) + ',' + str(acc[2][0]) + ',' + str(acc[2][1]) + ',' + str(acc[3][0]) + ',' + str(acc[3][1]) + ','  + str(acc1) + '\n')
