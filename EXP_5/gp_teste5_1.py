...
def import_all_data(files_paths, rand, test_percent):
	...
	for file_path in files_paths:
		...
		for row in data:
			if int(row[75]):
				X_S.append([float(x) for x in row[0:75]])
				y_S.append([int(row[75]), int(not(int(row[75])))])
			else:
				X_NS.append([float(x) for x in row[0:75]])
				y_NS.append([int(row[75]), int(not(int(row[75])))])
		csvfile.close()
		temp1 = []
		temp2 = []
...
def import_data(file_path, rand, test_percent):
	...
	for row in data:
		...
		y_train.append([int(row[75]), int(not(int(row[75])))])
	csvfile.close()
	if rand:
		temp1 = []
		temp2 = []
...
def main(NEXEC, K, TAM_MAX, NGEN, CXPB, MUTPB, NPOP, train_percent, verb, FILE_NAME):
	...
	files = ['wav_all_seg_ex1.csv', 'wav_all_seg_ex2.csv', 'wav_all_seg_ex3.csv', 'wav_all_seg_ex4.csv', 'wav_all_seg_ex5.csv', 'wav_all_seg_ex6.csv', 'wav_all_seg_ex7.csv', 'wav_all_seg_ex8.csv']
	X_train, y_train, X_test, y_test = import_all_data(files,1,.3)

	########## Operator Set #########################################
	pset = gp.PrimitiveSet("MAIN", 75)
	...
	#################################################################
	creator.create("FitnessMin", base.Fitness, weights=(1.0,5.0,1.0))
