# Módulo para pré-processamento e criação do dataset

# Variáveis globais
# Diretórios de entrada
PATH_IN_SCORE_EXPERTS <<- '/home/icaro/Desktop/TG/Dados_Sono/data/DREAMS_Sleep_Spindles/KComplex/Labels/'
PATH_IN_EDF_FILES     <<- '/home/icaro/Desktop/TG/Dados_Sono/data/DREAMS_Sleep_Spindles/KComplex/EDF/'

# Diretórios de saída
PATH_OUT_FEATURES     <<- '/home/icaro/Desktop/TG/Dados_Sono/SleepEEG/Exp/data/KC'
##################################################################################
...
#   5. scorer (int)     - Seleciona o diagnóstico de um especialista específico 
#                         (1 ou 2) ou de ambos (0).
#
# Retorno:
...
#
##################################################################################
generate_eeg_data <- function(excerpt, op, freq, freq_out, scorer){
  require(edf)
  require(readr)
  require(ppls)
  ...
  if(scorer == 2){
    exp1 <- read_table2(paste(PATH_IN_SCORE_EXPERTS, "Visual_scoring2_excerpt", excerpt, ".txt", sep = ""), col_types = cols(col_double(), col_double()))
  }else{
    exp1 <- read_table2(paste(PATH_IN_SCORE_EXPERTS, "Visual_scoring1_excerpt", excerpt, ".txt", sep = ""), col_types = cols(col_double(), col_double()))
  }
  
  if(excerpt < 6){
    if(scorer == 1){
      exp2 <- read_table2(paste(PATH_IN_SCORE_EXPERTS, "Visual_scoring1_excerpt", excerpt, ".txt", sep = ""), col_types = cols(col_double(), col_double()))
    }
    else{
      exp2 <- read_table2(paste(PATH_IN_SCORE_EXPERTS, "Visual_scoring2_excerpt", excerpt, ".txt", sep = ""), col_types = cols(col_double(), col_double()))
    }
  }
  excerpt_data <- read.edf(paste(PATH_IN_EDF_FILES, "excerpt", excerpt, ".edf", sep = ""))
  ...
  d = matrix(nrow = n_samples, ncol = 6)
  
  if(freq_factor > 1){
    d[,1] = sp1$x
    d[,2] = sp1$y
    d[,3] = spline(excerpt_data$signal$O1_A2$t, excerpt_data$signal$O1_A2$data, n=freq_factor*len, method = "fmm")$y
    d[,4] = spline(excerpt_data$signal$FP1_A2$t, excerpt_data$signal$FP1_A2$data, n=freq_factor*len, method = "fmm")$y
    d[,5] = 0
    d[,6] = 0
  }else{
    if(op == 1){
    ...
    d[,3] = excerpt_data$signal$O1_A2$data
    d[,4] = excerpt_data$signal$FP1_A2$data
    d[,5] = 0
  }
  c_exp1 = -1
  c_exp2 = -1
  ...
  d[c,5] = 1
  d[c_all,6] = 1
  print(paste('#K-Complex Events - Excerpt', excerpt))
  print(paste('intersect(Expert1, Expert2): ', round(sum(d[,5])*100/length(d[,5]),2), '% (', round(sum(d[,5])*100*900/length(d[,5]),2), ')', sep = ''))
  print(paste('union(Expert1, Expert2)    : ', round(sum(d[,6])*100/length(d[,6]),2), '% (', round(sum(d[,5])*100*900/length(d[,6]),2), ')', sep = ''))
  print('')
...
generate_eeg_data_all_excerpts <- function(freq_out, scorer){
  d <- NULL
  d$d1 <- generate_eeg_data(1,0,200,freq_out,scorer)
  d$d2 <- generate_eeg_data(2,0,200,freq_out,scorer)
  d$d3 <- generate_eeg_data(3,0,200,freq_out,scorer)
...
generate_eeg_data_all_excerpts_filter <- function(freq_out, scorer){
  ...
  d$d9 <- generate_eeg_data(9,0,200,freq_out,scorer)
  d$d9 <- filter_freq(d$d9,11,16)
  d$d10 <- generate_eeg_data(10,0,200,freq_out,scorer)
  d$d10 <- filter_freq(d$d10,11,16)
...
generate_features_all_excerpts <- function(d, freq, n_dwt, seg_len, stp, id, chs, nrm, folder){
  ...
  f$f9 <- generate_features(d$d9, freq, n_dwt, seg_len, stp, nrm, 9, id, chs, folder)
  f$f10 <- generate_features(d$d10, freq, n_dwt, seg_len, stp, nrm, 10, id, chs, folder)
  return(f)
}
