...
generate_eeg_data <- function(excerpt, op, freq, freq_out, scorer){
  ...
  d[c,5] = 1
  d[c_all,6] = 1
  print(paste('#Spindles Events - Excerpt', excerpt))
  print(paste('intersect(Expert1, Expert2): ', round(sum(d[,5])*100/length(d[,5]),2), '% (', sum(d[,5]), ')', sep = ''))
  print(paste('union(Expert1, Expert2)    : ', round(sum(d[,6])*100/length(d[,6]),2), '% (', sum(d[,6]), ')', sep = ''))
  print('')
