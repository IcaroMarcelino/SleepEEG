...
def main(NEXEC, K, TAM_MAX, NGEN, CXPB, MUTPB, NPOP, train_percent, verb, FILE_NAME):
	...
	def eval_tree1(individual, K, X_train, y_train, X_test, y_test, pset):
		...
		# evaluate accuracy
		#print accuracy_score(y_test, pred)
		return prf, accuracy_score(y_test, pred)
	...
	acc1, ac = eval_tree1(hof[0], K, X_train, y_train, X_test, y_test, pset)

	info.write(str(TAM_MAX) + ',' + str(K) + ',' +  str(NEXEC + 1) + ',' + str(ac) + ',' + str(hof[0].height) + ',' + str(end-start) + '\n')

	info1 = open("INFO_GP.csv", 'a')
	info1.write(str(TAM_MAX) + ',' + str(K) + ',' +  str(NEXEC + 1) + ',' + str(ac) + ',' + str(acc1[0][0]) + ',' + str(acc1[0][1]) + ',' + str(acc1[1][0])  + ',' + str(acc1[1][1]) + ',' + str(acc1[2][0]) + ',' + str(acc1[2][1]) + ',' + str(acc1[3][0]) + ',' + str(acc1[3][1]) + ','  + str(hof[0].height) + ',' + str(end-start) + '\n')
